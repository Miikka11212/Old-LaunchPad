...
struct LauncherLayout: Codable, Equatable {
    ...
    var customAppPaths: [String] = []
    enum CodingKeys: String, CodingKey { case entries, hiddenApps, customAppPaths }

    mutating func addApp(_ id: String, into folderID: String? = nil) {
        if !customAppPaths.contains(id) { customAppPaths.append(id) }
        hiddenApps.remove(id)
        if !contains(id) { entries.append(.app(id)) }
        if let folderID, folder(folderID) != nil { _ = move(id, before: nil, into: folderID) }
    }
...
extension LauncherLayout {
    init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        entries = try values.decode([LauncherEntry].self, forKey: .entries)
        hiddenApps = try values.decode(Set<String>.self, forKey: .hiddenApps)
        customAppPaths = try values.decodeIfPresent([String].self, forKey: .customAppPaths) ?? []
    }
}
