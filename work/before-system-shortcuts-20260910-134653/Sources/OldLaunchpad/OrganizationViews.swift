...
final class OrganizationViewport: PagingViewportView {
    ...
    override func mouseDown(with event: NSEvent) { emptyClick?() }
...
final class AppTileButton: NSButton, NSDraggingSource {
    ...
    private var geometry: TileGeometry {
        let width = (title as NSString).size(withAttributes: [.font: NSFont.systemFont(ofSize: 13, weight: .medium)]).width
        return TileGeometry(size: bounds.size, labelWidth: width)
    }
    override func hitTest(_ point: NSPoint) -> NSView? {
        guard let hit = super.hitTest(point), geometry.contains(convert(point, from: superview)) else { return nil }
        return hit
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        while let next = window.nextEvent(matching: [.leftMouseDragged, .leftMouseUp], until: .distantFuture, inMode: .eventTracking, dequeue: true) {
            ...
            if next.type == .leftMouseUp {
                highlight(false)
                if geometry.contains(point) { _ = sendAction(action, to: target) }
                return
            }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        displayIcon.draw(in: geometry.icon, from: .zero, operation: .sourceOver, fraction: isHighlighted ? 0.75 : 1)
        let paragraph = NSMutableParagraphStyle()
        paragraph.alignment = .center
        paragraph.lineBreakMode = .byTruncatingTail
        (title as NSString).draw(in: geometry.label, withAttributes: [
...
final class LauncherBackdropView: ClickSurfaceView {
    var outsideDrop: ((NSDraggingInfo, Bool) -> Bool)?
    var backgroundMenu: (() -> NSMenu)?
    override init(frame frameRect: NSRect) {
        super.init(frame: frameRect)
        registerForDraggedTypes([launcherItemPasteboardType])
    ...
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    override func menu(for event: NSEvent) -> NSMenu? { backgroundMenu?() }
    override func draggingEntered(_ sender: NSDraggingInfo) -> NSDragOperation { outsideDrop?(sender, false) == true ? .move : [] }
    override func draggingUpdated(_ sender: NSDraggingInfo) -> NSDragOperation { outsideDrop?(sender, false) == true ? .move : [] }
    override func prepareForDragOperation(_ sender: NSDraggingInfo) -> Bool { outsideDrop?(sender, false) ?? false }
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool { outsideDrop?(sender, true) ?? false }
}
